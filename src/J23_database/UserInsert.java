...
public class UserInsert {
	...
	public int saveUser(User user) {
		...
		try {
			...
			preparedStatement.setString(1, user.getUsername());   // 첫번째 ?에 문자열 "user.getUsername()" 을 넣는다.
			...
			resultSet = preparedStatement.getGeneratedKeys();  // db에서 자동완성된 키값을 가져오겠다.
															   // getGeneratedKey 는 Resultset형태다.
															   
			
			if(resultSet.next()) {                             //ResultSet.next 는 true, false를 리턴한다. 다음 값이 있으면 true
				System.out.println("이번에 만들어진 user_id Key값 : " + resultSet.getInt(1));   // getInt(1) 은 열의 위치를 가르킨다. 
	...
	public int saveRoles(Map<String, Object> map) {
		...
		String sql = null;		
		Connection connection = null;                     //연결할 때에는 필수 !! 핵심!
		PreparedStatement preparedStatement = null; 	  //연결할 때에는 필수 !! 핵심!
